...
class MainActivity : AppCompatActivity() {
    ...
    var currentNumberFormat: String = ""
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                withContext(Dispatchers.Main) { showToast("Güncelleme hatası: ${e.message}") }
            } finally {
    ...
    // Yeni Eklenen Metod: onActivityResult
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        super.onActivityResult(requestCode, resultCode, data)
        if (requestCode == MY_REQUEST_CODE) {
            if (resultCode != RESULT_OK) {
                Log.e("Update", "Güncelleme iptal edildi: $resultCode")
                checkUpdateAvailability()
            }
        }
    }
    ...
    private fun popupSnackbarForCompleteUpdate() {
        Snackbar.make(binding.root, "Güncelleme indirildi", Snackbar.LENGTH_INDEFINITE)
            .setAction("Yeniden Başlat") { appUpdateManager.completeUpdate() }
    ...
    fun checkUpdateAvailability() {
        appUpdateManager.appUpdateInfo
            .addOnSuccessListener { appUpdateInfo ->
                when {
                    appUpdateInfo.updateAvailability() == UpdateAvailability.UPDATE_AVAILABLE -> {
                        Snackbar.make(binding.root, "Yeni güncelleme mevcut", Snackbar.LENGTH_INDEFINITE)
                            .setAction("Yükle") {
            ...
            .addOnFailureListener { e -> showToast("Güncelleme hatası: ${e.message}") }
    }
    ...
    private fun configureActionBar() {
        supportActionBar?.apply {
            ...
            val scaledBitmap = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && logoDrawable is AdaptiveIconDrawable) {
                ...
                val bitmap = createBitmap(foreground.intrinsicWidth, foreground.intrinsicHeight)
                val canvas = android.graphics.Canvas(bitmap)
                foreground.setBounds(0, 0, canvas.width, canvas.height)
                foreground.draw(canvas)
                bitmap.scale(dpToPx(48), dpToPx(48))
            } else {
                if (logoDrawable is BitmapDrawable) {
                    logoDrawable.bitmap.scale(dpToPx(48), dpToPx(48))
                } else {
                    val bitmap =
                        createBitmap(logoDrawable.intrinsicWidth, logoDrawable.intrinsicHeight)
                    val canvas = android.graphics.Canvas(bitmap)
            ...
            setIcon(scaledBitmap.toDrawable(resources))
